...
class FOFENet(nn.Module):
    ...
    def __init__(self, block, emb_dims, channels, fofe_alpha=0.8, fofe_max_length=3, 
        ...
        self.embedding_fofe = self._make_layer(block, emb_dims, channels, 1, 4, fofe_alpha, fofe_max_length)
        self.model_fofe = self._make_layer(block, channels*4, channels, 4, 2, fofe_alpha, fofe_max_length)
        self.attention = Attention(channels, att_q2c, att_bidirection)
        self.output_encoder = self._make_layer(block, channels, channels, 3, 2, fofe_alpha, fofe_max_length, moduleList=True)
    ...
    def forward(self, query_emb, query_mask, doc_emb, doc_mask):
        ...
        q_code = self.embedding_fofe(query_emb)
        d_code = self.embedding_fofe(doc_emb)
        att_code = self.attention(d_code, q_code)
        model_code = self.model_fofe(att_code)
